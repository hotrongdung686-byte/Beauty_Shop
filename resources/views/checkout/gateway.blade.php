...
<x-shop-layout title="Thanh toán {{ $methodLabel }} - {{ config('app.name') }}">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div class="mt-6 space-y-3">
            @if(!$demo && $method === 'sepay' && $sepayCheckout)
                {{-- SePay's hosted checkout: submit the SDK-signed form directly. --}}
                <form action="{{ $sepayCheckout['action_url'] }}" method="POST">
                    @foreach($sepayCheckout['fields'] as $name => $value)
